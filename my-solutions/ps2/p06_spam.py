...
def fit_naive_bayes_model(matrix, labels):
    ...
    for row, y in zip(matrix, labels):
        if y == 1:
            nb_words_1 += row.sum() # orgianlly was len(row)  which was wrong!!!!!!!!!
        else:
            nb_words_0 += row.sum()
    ...
    for (row, col), elem in np.ndenumerate(matrix):
        if elem > 0 and labels[row] == 1:
            phi_1[col] += elem
        elif elem > 0 and labels[row] == 0:
            phi_0[col] += elem
    
    
    phi_1 = phi_1 / (nb_words_1 + len(phi_1))     
    phi_0 = phi_0 / (nb_words_0 + len(phi_1))
...
def predict_from_naive_bayes_model(model, matrix):
    ...
    for r_idx, rows in enumerate(matrix):
        ...
        for i, freq in enumerate(rows):
            if freq > 0:
                log_proc_1 += np.log(phi_1[i]) * freq
                log_proc_0 += np.log(phi_0[i]) * freq

        ratio = np.exp(log_proc_1 - log_proc_0)
        
        if ratio / (ratio + 1) > ((1 / ratio) / (1 + 1 / ratio)):
            res[r_idx] = 1
        else :
            res[r_idx] = 0
    return res
