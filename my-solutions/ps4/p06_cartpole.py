...
def initialize_mdp_data(num_states):
    ...
    """
    transition_counts = np.ones((num_states, num_states, 2))
...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    v_0 = np.sum(mdp_data['transition_probs'][state, :, 0] * mdp_data['value'])
    v_1 = np.sum(mdp_data['transition_probs'][state, :, 1] * mdp_data['value'])
    ...
    if v_0 > v_1:
        ...
        # wrong: return int(np.random.rand() >= 0.5)
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for x in range(mdp_data['num_states']):
        tran_total_0 = np.sum(mdp_data['transition_counts'][x, :, 0])
        tran_total_1 = np.sum(mdp_data['transition_counts'][x, :, 1])

        for y in range(mdp_data['num_states']):
            if mdp_data['transition_counts'][x, y, 0] != 0:
                mdp_data['transition_probs'][x, y, 0] = mdp_data['transition_counts'][x, y, 0] / tran_total_0

            if mdp_data['transition_counts'][x, y, 1] != 0:
                mdp_data['transition_probs'][x, y, 1] = mdp_data['transition_counts'][x, y, 1] / tran_total_1
    
        # update reward
        if mdp_data['reward_counts'][x, 0] != 0:
            mdp_data['reward'][x] =  - mdp_data['reward_counts'][x, 0] / mdp_data['reward_counts'][x, 1]
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while (max_change > tolerance):
        ...
        for x in range(mdp_data['num_states']):
            v_0 = np.sum(mdp_data['transition_probs'][x, :, 0] * mdp_data['value'])
            v_1 = np.sum(mdp_data['transition_probs'][x, :, 1] * mdp_data['value'])
            
            new_value = mdp_data['reward'][x] + gamma * max(v_0, v_1)
            diff = abs(new_value - mdp_data['value'][x])
            if diff > max_change:
                max_change = diff
            

            mdp_data['value'][x] = new_value
