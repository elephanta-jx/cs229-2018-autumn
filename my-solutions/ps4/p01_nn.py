...
def backward_softmax(x, grad_outputs):
    ...
    # dmax = np.where(exp == 0, 1, 0)
    # g = ((exp * (1 - dmax) * sum) - (exp ** 2 * (1 - dmax))) / (sum ** 2)

    # return grad_outputs * g

    # Compute the softmax function
    s = forward_softmax(x)
    
    # Create the Jacobian matrix for softmax
    jacobian = np.diag(s) - np.outer(s, s)
    
    # Compute the gradient: grad_outputs @ jacobian
    grad_input = np.dot(jacobian, grad_outputs)
...
def backward_relu(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    x_bar = np.where(x > 0, 1, 0)

    return grad_outputs * x_bar
    # *** END CODE HERE ***
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    # should use sum instead of mean
    grad_b = np.sum(output_grad, axis=(1, 2))

    w_1 = np.zeros((conv_channels, out_width, out_height, conv_width, conv_height))
    for c in range(conv_channels):
        for x in range(out_width):
            for y in range(out_height):
                w_1[c, x, y] = data[:, x:(x+conv_width), y:(y + conv_height)].copy() * output_grad[c, x, y]
    
    grad_w = np.sum(w_1, axis=(1, 2))

    in_1 = np.zeros((conv_channels, out_width, out_height, input_width, input_height))
    for c in range(conv_channels):
        for x in range(out_width):
            for y in range(out_height):
                
                in_1[c, x, y, x:(x+conv_width), y:(y+conv_height)] = output_grad[c, x, y] * conv_W[c, 0, :, :]
               
    
    grad_in = np.sum(in_1, axis=(0, 1, 2))
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    chan, out_width, out_height = output_grad.shape

    for c in range(chan):
        for x in range(out_width):
            for y in range(out_height):
                index = np.argmax(data[c, x * pool_width:x * pool_width + pool_width, y * pool_height:y * pool_height + pool_height])
                grad_data[c, x * pool_width:x * pool_width + pool_width, y * pool_height:y * pool_height + pool_height] \
                    [np.unravel_index(index, grad_data[c, x * pool_width:x * pool_width + pool_width, y * pool_height:y * pool_height + pool_height].shape)] = output_grad[c, x, y]
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    return (data.reshape(-1, 1) * output_grad, output_grad, weights.dot(output_grad))
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params):
    ...
    grad_entropy = backward_cross_entropy_loss(y, labels)
    grad_soft = backward_softmax(logits, grad_entropy)
    out_dir['W2'], out_dir['b2'], grad_linear = backward_linear(W2, b2, flattened, grad_soft)
    grad_relu = backward_relu(first_max_pool, grad_linear.reshape(first_after_relu.shape))
    grad_mp = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, grad_relu)
    out_dir['W1'], out_dir['b1'], _ = backward_convolution(W1, b1, data, grad_mp)
